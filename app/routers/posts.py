...
async def get_post(post_id: int,db:Session = Depends(get_db)):
    ...
    if post is None:
        # 見つからないときの例外を raise 404 Not found
...
# -----------------------
# スレッドの投稿一覧 GET /threads/(thread_id)/posts
# -----------------------
...
@threads_router.post("/{thread_id}/posts", response_class=HTMLResponse)
...
async def generate_dummy_posts(thread_id: int, count: int = 100, db: Session = Depends(get_db)):
    ...
    if thread is None:
        #見つからないときの例外をraise 404 Not found
        raise HTTPException(status_code=404, detail="Thread not found")
